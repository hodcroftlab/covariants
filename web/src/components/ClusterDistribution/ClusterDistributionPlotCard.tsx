...
import { clusterDisplayNameToLineageMapSelector, clusterDisplayNameToLineagesMapSelector } from 'src/state/Clusters'
...
export function ClusterDistributionPlotCard({
  ...
  const pangoName = pangoLineagesMap.get(clusterDisplayName)?.join(', ') ?? clusterDisplayName
