...
for fname in glob.glob("../covariants/travel_data/*xls"):
    ...
    travel_volume[country].index = [
        datetime.datetime(2020, i, 15) for i in range(1, 11)
    ]

d = pd.read_excel("../covariants/travel_data/UK.xls", skiprows=2).astype(str)
travel_volume["Wales"] = d.iloc[0, 3:].apply(
    lambda x: int(
        int(x.replace(".", "")) * popsizes["Wales"] / popsizes["United Kingdom"]
    )
)
...
q_free_to_spain = {
    ...
    "Netherlands": {
        "start": "2020-06-15",
        "end": "2020-08-24",
        "msg": "Netherlands to Spain",
    },
    "Denmark": {"start": "2020-06-27", "end": "2020-08-06", "msg": "Denmark to Spain"},
    "Denmark2": {
        "start": "2020-08-07",
        "end": fake_end_date,
        "msg": "Advised to quarantine, not required",
    },
}
...
q_free_to_other = {
    ...
    "Switzerland": {
        "start": "2020-06-15",
        "end": "2020-09-28",
        "msg": "Switzerland to UK",
    },
    "United Kingdom": {
        "start": "2020-07-10",
        "end": "2020-08-29",
        "msg": "UK to Switzerland",
    },
}

# note scotland was earlier
q_free_to_swiss = {"United Kingdom": {"start": "2020-07-10", "end": "2020-08-29"}}
