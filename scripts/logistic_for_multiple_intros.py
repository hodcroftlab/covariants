...
for want_node in intro_nodes:
    for node in cluster.find_clades(order="postorder"):
        if node.name == want_node:
            list_of_seqs[node_names[want_node]] = [
                leaf.name
                for leaf in node.get_terminals()
                if leaf.country == "Switzerland"
            ]
...
for coun in list_of_seqs.keys():
    dates = [
        datetime.datetime.strptime(meta[meta.strain == seq].date.values[0], "%Y-%m-%d")
        for seq in list_of_seqs[coun]
    ]
...
def non_zero_counts_intros(cluster_data, intro_id, country):
    cluster_and_total = pd.concat(
        [cluster_data[intro_id], total_data[country]], axis=1
    ).fillna(0)
...
for coun in list_of_seqs.keys():
    ...
    week_as_date, cluster_count, total_count = non_zero_counts_intros(
        intro_data, coun, intro_country
    )
    ...
    plt.plot(
        ...
        c=country_styles[fake_coun]["c"],
        ls=country_styles[fake_coun]["ls"],
