...
for clus in clus_to_run:
    ...
    cluster_meta_div = cluster_meta[
        cluster_meta["country"].apply(lambda x: x == selected_country)
    ]
...
def get_ordered_clusters_to_plot_div(clusters):
    ...
    clus_keys = [
        x
        for x in clus_keys
        if clusters[x]["type"] == "variant" or clusters[x]["usa_graph"] is True
    ]  # ["graphing"] is True]
    ...
    print(
        f"At min plot {min_to_plot}, there are {len(proposed_coun_to_plot)} divisions"
    )
