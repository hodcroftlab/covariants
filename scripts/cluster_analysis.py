...
def clean_metadata(Nextstrain_clades_display_names, acknowledgement_by_variant, alert_dates, clus_check, clus_data_all,
    ...
    if mode=='slow':
        ...
        # Filter for only Host = Human
        e = pl.scan_csv(input_meta, separator='\t')
        q = (
            e
            .select('Nextstrain_clade', 'country', 'host', 'QC_overall_status', 'date', 'coverage')
            .filter(pl.col('host').ne('Human'))
            # Filter only for those without 'bad' and those without '' QC status
            # available values (as of 30 mar 22) are 'bad', 'good', 'mediocre', ''
            # these '' values are likely those where alignment fails
            # as of 30 mar 22, this excludes 466,596 sequences (!) for bad, and 4,781 for ''
            # .filter(pl.col('QC_overall_status').is_in(['bad', '']).not_())
            # .filter(pl.col('date').len().eq(10) | pl.col('date').str.contains('X'))
            # .filter(pl.col('strain').is_in(bad_seqs).not_() | pl.col('date').ne(bad_seqs[pl.col('strain')]))
            # .filter(pl.col('coverage').ge(0.9) | pl.col('coverage').ne('?'))
            # as of 7 Dec 2023 - if clade is in list of new clades that need renaming, replace clade
            # .with_columns(pl.col('Nextstrain_clade').map_elements(lambda clade: new_clades_to_rename.get(clade, clade)))
        )

        data = q.collect()
        noQC = e.select('coverage').filter(pl.col('coverage').eq('?')).count().collect()
