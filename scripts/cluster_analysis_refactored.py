...
}  # should be in format of Nextstrain_clade or column used to decide clade (Currently "23A (Omicron)", example entry: "24A (Omicron)": "23I (Omicron)")
...
def print_all_date_alerts(alert_dates, cluster_first_dates):
    for clus in alert_dates.keys():
        print_date_alerts(clus, alert_dates, cluster_first_dates)
        print("\n")


# Print out bad sequences in a format that can be directly copied over to bad_sequences.py
def print_bad_sequences(alert_dates, cluster_first_dates):
    print()
    for clus in alert_dates.keys():
...
def get_instructions_from_user(clusters):
    """Terminal interaction to ask user if they want to write-out files, acknowledgments and which clusters to run"""
    print_files = True
    ...
    # # Full country plotting and division: collect division info for the USA and Switzerland
...
def clear_output_files():
    ...
    current_path = cluster_path + "clusters/current/"
    for f in os.listdir(current_path):
        if os.path.isfile(current_path + f):
            os.remove(current_path + f)


def read_metadata_file_first_run(input_meta, cols):
    ...
    with open(input_meta) as f:
        header = f.readline().rstrip().split("\t")  # TODO: check if rstrip is really necessary
    ...
    return nextstrain_clades, all_countries, n_total


def read_and_clean_metadata_file_line_by_line(clusters, input_meta, cols, n_total, selected_country,
                                              nextstrain_clades, display_name_to_clus,
                                              pango_lineage_to_clus,
                                              clus_check,
                                              dated_limit, dated_cluster,
                                              print_acks, clus_to_run, bad_seqs,
                                              swiss_regions, cluster_first_dates, first_date_exceptions, all_countries):
    t0 = time.time()
    ...
    alert_dates = {}  # All strains that are dated earlier than their respective clade are auto-excluded and printed out

    nextstrain_clades_display_names = extract_display_names(nextstrain_clades, clus_to_run, display_name_to_clus)

    clus_to_run_breakdown, rest_all = split_clusters_into_categories(nextstrain_clades, clus_to_run, clusters)

    daughter_clades = prepare_daughter_clades(clus_to_run, clusters)

    acknowledgement_by_variant = prepare_acknowledgements_dictionary(clus_to_run)

    division_data_all = prepare_division_summary_table(clus_to_run, selected_country)

    clus_data_all = prepare_output_dictionary(all_countries, clus_to_run, clusters)

    total_counts_countries, total_counts_divisions = prepare_counts_dictionaries(all_countries, division_data_all)

    print("\nReading and cleaning up the metadata line-by-line...\n")
    n = 0
    no_qc = 0
    with open(input_meta) as f:
        header = f.readline().rstrip().split("\t")  # TODO: check if rstrip is necessary
        indices = {c: header.index(c) for c in cols}
        ...
        while True:
            ...
            date_2weeks = helpers.to2week_ordinal(date_formatted)
            ...
            for clus in clus_all:
                ...
                # if we want a dated limit, specify cluster and limit at top
    ...
    return cluster_inconsistencies, all_sequences, alert_dates, acknowledgement_by_variant, total_counts_countries, total_counts_divisions, clus_data_all, division_data_all, rest_all, dated_cluster_strains


def plot_country_data(
        clusters,
        proposed_countries_to_plot,
        clus_keys,
        division_data_all,
        clus_data_all,
    ...
    min_week = datetime.datetime.today()
    max_week = earliest_date
    week_as_dates = {}
    json_output = {"countries": {}}

    for country in proposed_countries_to_plot:
        i = 0
        first_clus_count = []
...
def print_color_info(countries_to_plot, country_styles_all):
    ...
    for country in countries_to_plot:
        if country not in country_styles_all:
            print(
                f"WARNING!: {country} has no color! Please add it to country_list_2 in colors_and_countries.py and re-run make web-data.")
...
def compile_meta_clusters(acknowledgement_by_variant, all_sequences, clus_data_all, clusters, display_name_to_clus,
                          meta_clusters, print_acks):
    ...
    for meta_clus in meta_clusters:
        for disp in clusters[meta_clus]["other_nextstrain_names"]:
            ...
            for country in clus_data_all[clus]["summary"]:
                ...
                clus_data_all[meta_clus]["summary"][country]["num_seqs"] += clus_data_all[clus]["summary"][country][
                    "num_seqs"]
                clus_data_all[meta_clus]["summary"][country]["first_seq"] = min(
                    clus_data_all[meta_clus]["summary"][country]["first_seq"],
                    clus_data_all[clus]["summary"][country]["first_seq"])
                clus_data_all[meta_clus]["summary"][country]["last_seq"] = max(
                    clus_data_all[meta_clus]["summary"][country]["last_seq"],
                    clus_data_all[clus]["summary"][country]["last_seq"])
        ...
        if print_acks:
            for disp in clusters[meta_clus]["other_nextstrain_names"]:
                if disp not in display_name_to_clus:
                    ...
                    continue
                clus = display_name_to_clus[disp]
                acknowledgement_by_variant["acknowledgements"][meta_clus].extend(
                    acknowledgement_by_variant["acknowledgements"][clus])
...
def pass_helper_functions_over_data(all_countries, clus_data_all, division_data_all, selected_country,
    ...
    for clus in clus_data_all:
        ...
        for country in all_countries:
            ...
            ) = helpers.non_zero_counts(cluster_data, total_data, country)

            if len(total_count) < 2:
                continue

            week_as_date, cluster_count, total_count = helpers.trim_last_data_point(
        ...
        for clus in division_data_all[country]:
            ...
            for div in division_data_all[country][clus]["cluster_counts"]:
                ...
                (
                    ...
                    unsmoothed_cluster_count,
                    unsmoothed_total_count,
                ) = helpers.non_zero_counts(cluster_data, total_data, div)
    ...
    return clus_data_all, division_data_all


def print_number_of_bad_sequences(alert_dates):
    ...
    if alert_dates:
        ...
        print(
            "To view, use 'print_all_date_alerts()' or 'print_date_alerts(<clus>)'. Formatted for bad_sequences.py: use 'print_bad_sequences()'\n")
    else:
        print("\nNo bad dates found.\n")


def print_inconsistent_cluster_assignments(cluster_inconsistencies, rest_all):
...
def print_cluster_counts(all_sequences, clusters):
    ccounts = []
...
def collect_countries_to_plot(clus_data_all, cutoff_num_seqs):
    ...
    for clus in clus_data_all:
        for country in clus_data_all[clus]["summary"]:
            if clus_data_all[clus]["summary"][country][
                "num_seqs"] > cutoff_num_seqs and country not in countries_to_plot:
...
def check_for_meta_clusters(clus_to_run, clusters):
    changed_clus_to_run = False
    meta_clusters = [clus for clus in clus_to_run if
                     "meta_cluster" in clusters[clus] and clusters[clus]["meta_cluster"]]
...
def write_summary_tables(run_all_clusters, clus_data_all):
    ...
    for clus in clus_data_all:
        ...
        ordered_country = pd.DataFrame.from_dict(clus_data_all[clus]["summary"], orient="index").sort_values(
            by=["first_seq", "last_seq"])
...
def format_cluster_first_dates(cluster_first_dates):
    for clus in cluster_first_dates:  # Transform date from string to datetime for easier comparison
        cluster_first_dates[clus]["date_formatted"] = datetime.datetime.strptime(
            cluster_first_dates[clus]["first_date"], "%Y-%m-%d")

    return cluster_first_dates


def prepare_acknowledgements_dictionary(clus_to_run):
    # store acknowledgements
    acknowledgement_by_variant = {"acknowledgements": {clus: [] for clus in clus_to_run}}
    return acknowledgement_by_variant


def prepare_counts_dictionaries(all_countries, division_data_all):
    ...
    return total_counts_countries, total_counts_divisions


def prepare_output_dictionary(all_countries, clus_to_run, clusters):
...
def prepare_division_summary_table(clus_to_run, selected_country):
    ...
    return division_data_all


def create_name_mappings(clus_to_run, clusters):
    ...
    nextstrain_name_to_clus = {clusters[clus]["nextstrain_name"]: clus for clus in clus_to_run if
                               "nextstrain_name" in clusters[clus]}
    ...
    return display_name_to_clus, pango_lineage_to_clus


def extract_display_names(nextstrain_clades, clus_to_run, display_name_to_clus):
    ...
    return nextstrain_clades_display_names


def prepare_daughter_clades(clus_to_run, clusters):
    ...
    return daughter_clades


def split_clusters_into_categories(nextstrain_clades, clus_to_run, clusters):
    ...
    for clus in clus_to_run:
        for key in snps_categories:
            if key in clusters[clus] and clusters[clus][key]:
                if (clusters[clus]["display_name"] in nextstrain_clades) or (
                        "meta_cluster" in clusters[clus] and clusters[clus]["meta_cluster"]) or (
                        "use_pango" in clusters[clus] and clusters[clus]["use_pango"]):
    ...
    return clus_to_run_breakdown, rest_all


def get_ordered_clusters_to_plot(clusters, division_data_all, clus_data_all, division_local=False,
                                 selected_country_local=None):
    ...
    for clus in clus_keys:
        ...
        for c in country_info:
            if country_info[c]["num_seqs"] > min_to_plot and c not in proposed_countries_to_plot:
                proposed_countries_to_plot.append(c)

        # special rule for Andorra
        if "Andorra" in country_info and country_info["Andorra"][
            "num_seqs"] > 50 and "Andorra" not in proposed_countries_to_plot:
            proposed_countries_to_plot.append("Andorra")

    if division_local:
        print(
            f"\nDivision plotting ({selected_country_local}): At min plot {min_to_plot}, there are {len(proposed_countries_to_plot)} entries")
    else:
        print(
            f"\nCountry plotting: At min plot {min_to_plot}, there are {len(proposed_countries_to_plot)} entries PLUS ANDORRA")
    ...
    for clus in clus_keys:
        ...
        for country in country_info:
            if country in proposed_countries_to_plot:
                if country not in total_country_counts:
                    total_country_counts[country] = 0
                total_country_counts[country] += country_info[country]["num_seqs"]

    sorted_country_tups = sorted(total_country_counts.items(), key=lambda x: (-x[1], x[0]))
    proposed_countries_to_plot = [x[0] for x in sorted_country_tups]

    return proposed_countries_to_plot, clus_keys


if __name__ == '__main__':
    from colors_and_countries import country_styles_all as country_styles
    from clusters import clusters as cluster_data
    from bad_sequences import bad_seqs as bad_sqs
    from approx_first_dates import cluster_first_dates as cluster_first_dates_raw, \
        first_date_exceptions as date_exceptions
    from swiss_regions import swiss_regions as sw_regions
