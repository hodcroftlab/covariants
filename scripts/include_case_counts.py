...
import datetime
import sys
from scripts.helpers import to2week_ordinal_string
...
def main(owid_csv_input_path, country_csv_input_path, output_csv_path, threshold=THRESHOLD, period_pass=PERIOD_PASS):
    ...
    with open(country_csv_input_path) as f:
        per_country_data = json.load(f)

    world_data = per_country_data["regions"][0]["distributions"]
    per_country_intro_content = per_country_data["regions"][0]["per_country_intro_content"]
    max_date = per_country_data["regions"][0]["max_date"]
    min_date = per_country_data["regions"][0]["min_date"]
    cluster_names = per_country_data["regions"][0]["cluster_names"]

    world_data_counts = []

    for i in range(len(world_data)):
        country = world_data[i]["country"]
        if country not in owid_grouped["location"].values and country not in alternative_country_names:
            print("Attention! Country not found in owid data: " + country)
            continue

        country_owid = country
        if country in alternative_country_names:
            country_owid = alternative_country_names[country]
